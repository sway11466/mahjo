...
魅力的なキャラクターと一緒に麻雀の学習をしよう ── 役と点数の基礎が身につく個人用の学習Webアプリ。**役モード・点数モードが動作する実装済みアプリ（未実装・残作業は [docs/backlog.md](docs/backlog.md)）。**
...
未決の設計事項は本文のその場に **TBD** と記す（`grep -rn TBD docs/` で一覧できる）。「後回しの作業アイデア」は [docs/backlog.md](docs/backlog.md) の parking lot に置く（**未決の判断と未着手の作業は別物**＝TBD は判断待ち、parking lot はやると価値があるが今やらない作業）。

## Claude の行動規則（厳守）

- 実装・コマンド実行・git 操作は**許可を得てから**行う。サイレントなコミット/push は禁止（提案は「〇〇してよいですか？」にとどめる）。
- **ファイル編集を始める前に、ユーザーから別段の指示がなければ EnterWorktree でワークツリーを作ってから編集する。**「ワークツリー不要」「ここで直接」等の指示があればそれに従う。
