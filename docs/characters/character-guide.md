...
立ち絵（全身）の等身（頭身＝頭頂〜あご ÷ 全身高）は**キャラごとに違ってよい**（個性・世代を映す）。並んだときの一体感は**バストアップの顔 scale**で作る（§4「作る画像セットと順序」のクロップノート＝既存キャラ neutral と並べて合わせる）ので、全身の等身は無理に一致させない。ただし完全に自由ではなく**世代で帯がある**＝採用前に次をレビューする：

- **世代で段が付くのが正**：弟子世代（10代）より師匠世代（大人）が高等身。格・年齢が等身で読めるのが望ましい（[decisions](../decisions.md) の「格が上がるほど役も大きい」グラデーションの等身版）。目安帯（確定キャラ基準）：
  - 弟子世代（10代）＝約 **6.5〜7.5頭身**。**同年齢帯でも絵の見せ方で幅を持たせてよい**（例：まお＝低め＝3人で一番幼い演出／りん・しん＝高め＝スラッと。3人とも設定上は同年齢帯）。
  - 師匠世代（大人）＝約 **7.5〜8頭身**（例：ひすい）。弟子より一段高く、世代差が等身で読める。
- **レビューで見る点**：(1) 同世代の既存キャラと並べ、等身帯から外れていないか（弟子なのに大人8頭身／師匠なのに幼い等）。(2) 大人に倒れすぎ・幼くなりすぎていないか。(3) ポーズ（膝曲げ等）や床丈の衣装で**見かけの高さ**が変わるので、身長でなく**頭/身の比率**で判定する。
- **顔サイズは別軸**：全身の等身と顔 scale は別問題。顔の大きさは全身でなく**バストアップ生成時**に既存キャラ neutral と並べて合わせる（§4「作る画像セットと順序」）。
